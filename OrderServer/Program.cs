...
namespace OrderServer
{
    class Order : IComparable<Order>
    {
        ...
        public static Order Parse(string str)
        {
            ...
            order.id = long.Parse(statement[0]);
        ...
        public bool SelectProducer(string[] producer)
        {
            foreach (OrderItem item in items)
            {
                ...
                {
                    if (item.producer == name) return true;
                }
            }
            return false;
        }
        ...
        public int CompareTo([AllowNull] Order other)
        {
            ...
            if (c == 0) return id.CompareTo(other.id);
            return c;
        }
    }
    ...
    class OrderService
    {
            ...
            for(; ; )
            {
                ...
                else
                {
                    ...
                    if (statement[0].ToLower() == "id")
                    {
                        if (operation == 1)
                        {
                            temp = (from m in list where m.id < long.Parse(statement[2]) orderby m.price select m);
                            list = temp.ToList();
                            list.Sort();
                        }
                        else if (operation == 2)
        ...
        void UpdateProgram()
        {
            ...
            {
                string[] statement = line.Split(' ');
                if (table[long.Parse(statement[0])] != null)
                    table[long.Parse(statement[0])] = Order.Parse(line);
        ...
        public void Excute()
        {
            for (; ; )
            {
                try
                {
                    ...
                    {
                        Console.WriteLine("H: help, T:show table, A: add, S: select, C: clear, U: update");
                    }
    ...
    {     
        ...
        {
            Console.WriteLine("Welcome to Order System v0.1.1 by UPiracy.");
            OrderService servise = new OrderService();
            servise.Excute();
            Console.WriteLine("You exit Order System v0.1.1.");
            Console.ReadLine();
        }
    }
}
